...
- `ignorecache` and `invalidate` are both gated on the `x-invalidate-key` header against `invalidate_token`. An unauthenticated `ignorecache` is *neutralized*, not rejected: the request is served from cache as normal rather than 403ing, so a stray benign caller still gets its image.
- Every upstream fetch a request makes shares one wall-clock budget (`FETCH_DEADLINE_MS`, default 25s, overridable via `fetch_deadline_ms`). The mirror walk stops rather than starting a candidate it cannot finish, and a walk truncated that way only ever earns the short 60s negative-cache TTL because it learned nothing conclusive. The default-image fetch is reserved budget outside the deadline, so an exhausted chain still ends as a 200 placeholder. Retune without a rebuild by restarting with `NODE_CONFIG='{"fetch_deadline_ms":40000}'` — `config/` is baked into the image.
