...
- Animated sources (src/animated.ts): a GIF or animated WebP over `animated_passthrough_max_size` (100KB) is re-rendered at the requested size, as animated WebP for a client that takes WebP and as GIF otherwise. Never to AVIF/PNG/JPEG: handed a multi-page pipeline libvips writes the frames as one tall "filmstrip" still. Only when libvips counted the frames itself (`pages > 1`): it does not read APNG animation, so an APNG loads as one page and keeps the passthrough. The render is served only if the output container declares the same frame count as the source and is smaller; otherwise the source bytes go out untouched. Frames are counted off the container, not by asking libvips again, since libvips dropping frames is the thing being guarded against
- `?blur=1` LQIP placeholders render from the first frame of an animation, never from every frame
...
- Proxy store: Sharded filesystem (src/sharded-fs-store.ts) — distributes files into subdirectories by the first 6 chars of the key (`SHARD_LEN`), backwards-compatible with flat layout via automatic migration. This is the resize/proxy cache and is safe to LRU-prune. A miss writes **two** entries: the rendered variant under `imageKey`, and (optionally) the untouched original under `origKey`. The original is only a shortcut for re-rendering a *different* variant without a second upstream fetch, and it is the expensive half — on the live store the median file is 69KB but the 10% over 1MB hold 67% of the bytes. `shouldCacheOriginal()` in src/proxy.ts skips originals above `max_cached_original_size` (default 1MB, 0 disables) so the cache is not dominated by a long tail of large sources
...
- Default/fallback images are never stored persistently, in **any** handler. Bytes travel as a `ServedImage` (`src/served-image.ts`) whose `kind` is `real` or `fallback`: a render inherits its source's kind (`derive`), a real fetch inside a substituted request is still a fallback (`worstOf`), and `storeImage` refuses the fallback kind so no call site has to remember the check. Cache-Control (`cacheControlFor`: 120s for a fallback) and the ETag (`etagFor`: a fallback gets its own deterministic ETag, never the key's) are derived from the value, not from a flag beside it. Storing a fallback would be indistinguishable from a real image on later requests and would be served at the normal freshness; sharing the real ETag would let a cache revalidate a placeholder against the recovered image. The one deliberate exception is a *profile-lookup* fallback (`isProfileFallback`, a transient RPC miss): it derives both keys from the default image's own URL, so it never occupies a user's key and is safe to cache; the response still carries the fallback header and ETag
...
Tests in `test/` directory use Mocha + ts-node. Key test files:
...
- `test/animated.ts` - Animated sources keep their frames (fixture GIF generated by `test/animated-gif-fixture.ts`, since this sharp cannot write an animation)
...
- `ignorecache` and `invalidate` are both gated on the `x-invalidate-key` header against `invalidate_token`. An unauthenticated `ignorecache` is *neutralized*, not rejected: the parameter is dropped and the request resumes normal cache handling rather than 403ing, so a stray benign caller still gets its image. This is not a guaranteed cache hit — a genuine miss still fetches and renders, and Varnish passes these requests to the backend regardless. What the gate removes is the ability to force a miss on a variant that *is* cached. Blocking it at the edge would be separate configuration work.
- Every upstream fetch a request makes shares one wall-clock budget (`FETCH_DEADLINE_MS`, default 25s, overridable via `fetch_deadline_ms`). The mirror walk stops rather than starting a candidate it cannot finish, and a walk truncated that way only ever earns the short 60s negative-cache TTL because it learned nothing conclusive. The default-image fetch is reserved budget outside the deadline, so an exhausted chain still gets to *attempt* the placeholder; that fetch has its own timeouts and can itself fail, in which case the request still ends in an error. Retune without a rebuild by restarting with `NODE_CONFIG='{"fetch_deadline_ms":40000}'` — `config/` is baked into the image.
